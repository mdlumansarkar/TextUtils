    ...
    <>
      <div>
        <div  className="mb-3 container">
          ...
          <button className="btn btn-primary my-3" onClick={handleClickUp}>Convert to Uppercase</button>
          <button className="btn btn-danger my-3 mx-3" onClick={handleClickLo}>Convert To Lowercase</button>

        </div>
      </div>
      <div className="container my-3">
        <h2>Your Text Summary</h2>
        <p>You Type {text.split(" ").length} words and {text.length} cahracters</p>
        <p>You need to take time to read this {text.length*0.004} minutes</p>
        <p>your text here : <br/>{text}</p>
      </div>
    </>
